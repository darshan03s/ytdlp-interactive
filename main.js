...
import { createVideoInfoJsonFromYtDlp, getFormatsFromInfoJson } from "./utils/ytdlpUtils.js";
import { printDivider, printDividerWithText } from "./utils/shellUtils.js";
import { APP_FOLDER, SETTINGS, SETTINGS_PATH, DOWNLOADS_DATA_FOLDER, setAppSettings, INFO_JSON_PATH } from "./globals.js";
...
async function init() {
    if (isSettingsPresent(SETTINGS_PATH)) {
        ...
        if (isYtDlpInstalled && isFfmpegInstalled) {
            ...
            await createSettings(SETTINGS_PATH, APP_FOLDER, DOWNLOADS_DATA_FOLDER);
            await updateSettings();
        }
    }
}
...
if (parseDotDate(latestYtDlpVersion) > parseDotDate(SETTINGS.ytdlp_version)) {
    ...
    console.log(chalk.yellowBright(`Download latest version from: https://github.com/yt-dlp/yt-dlp/releases or use yt-dlp -U`));
} else {
...
async function getAndValidateUserInput(defaultInput = "") {
    const userEnteredUrl = await input({ message: 'Enter URL: ', default: defaultInput });
...
async function getSelectedDownloadLocation() {
    ...
    const useDefaultDownloadLocation = await confirm({ message: 'Use default download location?' });
    if (!useDefaultDownloadLocation) {
        ...
        const setDefaultDownloadLocation = await confirm({ message: 'Set this download location as default?' }, { defaultValue: false });
...
try {
    while (true) {
        ...
        console.log();
        console.log(chalk.blueBright(`URL History: ${SETTINGS.url_history.length}`));
        console.log(chalk.blueBright(`Download Location History: ${SETTINGS.download_location_history.length}`));
        ...
        console.log();
        let userInputArg;
        if (process.argv.length > 2) { userInputArg = process.argv[2] }
        ...
        spinner = new yoctoSpinner({ text: 'Fetching video metadata...' }).start();
        ...
        // ----- START DOWNLOAD -----
        console.log();
        const confirmStartDownload = await confirm({ message: 'Start Download?' });
